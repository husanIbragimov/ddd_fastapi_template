from uuid import UUID

from injector import inject, singleton

from application.dto import TagDTO, PagingDTO
from application.mappers import tag_dto_to_entity, tag_entity_to_dto
from core.exceptions import ApplicationException
from core.response import ApiResponse
from domain.repository import TagRepository


@singleton
class TagUseCase:

    @inject
    def __init__(self, repository: TagRepository):
        self.repository = repository

    async def create(self, dto: TagDTO) -> UUID | ApiResponse[None]:
        try:
            rag_entity = tag_dto_to_entity(dto)
            result = await self.repository.create(rag_entity)
            return result
        except ApplicationException as err:
            return ApiResponse.error_response(
                message=f"Internal Server Error: {err}",
                error_code=500
            )

    async def get_by_id(self, tag_id: UUID) -> ApiResponse[TagDTO | None]:
        result = await self.repository.get_by_uuid(tag_id)
        if result is None:
            return ApiResponse.error_response(
                message="Tag not found",
                error_code=404
            )
        return ApiResponse.success_response(
            data=tag_entity_to_dto(result),
            message="Tag found successfully"
        )

    async def list(self, skip: int = 0, limit: int = 10) -> PagingDTO[TagDTO]:
        result = await self.repository.list(skip=skip, limit=limit)
        return PagingDTO.new(
            page=result.page,
            size=result.size,
            total=result.total,
            items=[tag_entity_to_dto(item) for item in result.items]
        )

    async def update(self, tag_id: UUID, dto: TagDTO) -> UUID | ApiResponse[None]:
        try:
            tag_entity = tag_dto_to_entity(dto)
            result = await self.repository.update(tag_id, tag_entity)
            return result
        except ApplicationException as err:
            return ApiResponse.error_response(
                message=f"Internal Server Error: {err}",
                error_code=500
            )
